...
HiLassoTRI <- function(x, y, bootstraps, alpha = c(0.5, 1), box.width,
    ...
    if (cores < 2) {
        list.beta.hat2 <- lapply(seq_len(bootstraps), .part2, x, y, as.numeric(Sys.time()))
    } else {
        list.beta.hat2 <- mclapply(seq_len(bootstraps), .part2, x, y, as.numeric(Sys.time()), mc.cores = cores)
    }
    
    importance.measure2 <- Reduce('+', lapply(list.beta.hat2, abs)) + 10E-10 # Should release from memory.
    
    .part3 <- function(ii, x, y, start_time) {
        if (verbose) {
            .helper.time.remaining(pb, start_time, ii, bootstraps)
        }
        
        random.features <- sample(number.of.features, box.width, replace = FALSE,
                                  prob = importance.measure2)
        random.samples <- sample(number.of.samples, replace = TRUE)
    ...
    if (cores < 2) {
        list.beta.hat3 <- lapply(seq_len(bootstraps), .part3, x, y, as.numeric(Sys.time()))
    } else {
        list.beta.hat3 <- mclapply(seq_len(bootstraps), .part3, x, y, as.numeric(Sys.time()), mc.cores = cores)
    }
    ...
    reduced.beta.hat <- Reduce('+', list.beta.hat3) / bootstraps
