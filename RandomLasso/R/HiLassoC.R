...
HiLassoC <- function(x, y, bootstraps, alpha = c(0.5, 1), box.width,
                    nfold = 5, cutoff = c(0, 0, 0), cores = FALSE, verbose = TRUE, test = FALSE) {
    ...
    if (cores == TRUE) { # This causes inconsistent failures when TRUE...
        cores <- detectCores()
    }
    ...
    .part1 <- function(ii, x, y, start_time) {
        ...
        beta.hat[abs(beta.hat) < cutoff[1]] <- 0
        return(beta.hat)
    }
    ...
    .part2 <- function(ii, x, y, start_time) {
        ...
        beta.hat[random.features] <- AdaptiveLasso(random.x.scaled,
                                                   ...
                                                   random.importance,
                                                   nfold) / standard.deviation
        beta.hat[abs(beta.hat) < cutoff[2]] <- 0
        return(beta.hat)
    }
    ...
    reduced.beta.hat[abs(reduced.beta.hat) < cutoff[3]] <- 0
