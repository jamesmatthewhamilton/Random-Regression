...
# Export one simulation data to .RData binary files.
x <- y <- x_val <- y_val <- x_cov <- list()
x <- matrix(unlist(sim_data[[1]][3]),
                    ncol = n_feature, byrow = FALSE)
y <- matrix(unlist(sim_data[[1]][2]), ncol = 1)
x_val <- matrix(unlist(sim_data[[1]][7]),
                        ncol = n_feature, byrow = FALSE)
y_val <- matrix(unlist(sim_data[[1]][6]), ncol = 1)
x_cov <- matrix(unlist(sim_data[[1]][8]),
                       ncol = n_feature, byrow = FALSE)
beta0 <- as.matrix(beta0)
save(x, y, x_val, y_val, x_cov, beta0, file = "data/sim4/sim4_1.RData")
