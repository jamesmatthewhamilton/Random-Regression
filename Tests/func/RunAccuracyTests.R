...
RunAccuracyTest <- function(s, dir, title, tests, iterations = 10, column.names) {
    ...
    write.csv(x = f1.b, paste0(directory, "/F1", rows.cols, ".csv"))
    write.csv(x = f1.b.cutoff, paste0(directory, "/Cutoff_F1", rows.cols, ".csv"))
    write.csv(x = f1, paste0(directory, "/Pure_F1", rows.cols, ".csv"))
